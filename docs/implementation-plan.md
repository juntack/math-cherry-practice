...
MVPの主要機能と表示改善はフェーズ5までで完了扱いとする。フェーズ6では、新しい学習機能の追加ではなく、Android端末とiOS/iPadOS端末でオフライン利用できる形にすることを目的にする。
...
- Safariでホーム画面に追加できるよう、Web App Manifestを用意する
- iOSのホーム画面用アイコンを用意する
- Service Workerでビルド済みファイルをキャッシュする
- 初回アクセス後、オフラインでも起動できることを確認する

現状では、PWA用のmanifest、アイコン、Service Worker生成処理を追加済み。
